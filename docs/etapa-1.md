...
- Avaliações e coleta autenticada desabilitadas por padrão. O coletor local verifica certificados HTTPS. Sua disponibilização pública continua dependendo da definição da fonte e das condições de uso.

## Validação

72 testes aprovados: 54 cenários herdados, dez regressões da auditoria e oito verificações complementares. Duas expectativas antigas foram atualizadas com mudança explícita de contrato: sobreposição agora exige o mesmo código na fixture; um Pareto completo pode existir mesmo quando o pool padrão foi limitado, se uma alternativa domina a outra.
